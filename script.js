...
function printHtml(htmlContent, fileName) {
    ...
    var css = [
        '<style>',
        '*{box-sizing:border-box;}',
        '#_pbar{position:fixed;top:0;left:0;right:0;display:flex;gap:5px;padding:9px 10px;background:#1a365d;z-index:2147483647;box-shadow:0 3px 10px rgba(0,0,0,.5);}',
        '#_pbar button{flex:1;padding:12px 2px;border:none;border-radius:10px;font-size:.88rem;font-weight:800;cursor:pointer;font-family:Tahoma,Arial,sans-serif;}',
        ...
        '@media screen{body{padding-top:60px!important;}}',
        '</style>'
    ].join('');
    ...
    var sc = 'var _fn=' + fnStr + ';'
        ...
        + 'navigator.share({files:[file],title:_fn}).catch(function(){_dl(html);});'
        + '}else{_dl(html);}'
        + '}'
        + 'function _dl(h){'
        + 'try{'
        + 'var b=new Blob([h||document.documentElement.outerHTML],{type:"text/html;charset=utf-8"});'
    ...
    var injected = css + '<scr' + 'ipt>' + sc + '<' + '/scr' + 'ipt>';

    var bar = '<div id="_pbar">'
        + '<button id="_pbtn" onclick="window.print()">چاپ</button>'
        + '<button id="_dbtn" onclick="_share()">داونلۆد</button>'
        + '<button id="_cbtn" onclick="' + closeCode + '">✕</button>'
        + '</div>';

    var final = cleaned;
    if (/<\/head>/i.test(final)) {
        final = final.replace(/<\/head>/i, injected + '</head>');
